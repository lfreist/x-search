...
std::shared_ptr<count_matches> extern_search(const std::string& pattern,
                                             ...
                                             bool ignore_case,
                                             int num_threads) {
  std::unique_ptr<task::reader::FileReader<DataChunk>> reader;
  if (ignore_case) {
    reader = std::make_unique<task::reader::FileBlockReader>(file_path);
  } else {
    reader = std::make_unique<task::reader::FileBlockReaderMMAP>(file_path);
  }
  ...
  return std::make_shared<count_matches>(
      ...
      std::move(searcher), std::make_unique<result::CountMatchesResult>());
}
...
std::shared_ptr<count_lines> extern_search(const std::string& pattern,
  ...
  // construct the ExternSearcher and return it as shared_ptr
  return std::make_shared<count_lines>(
      num_threads, std::move(reader), std::move(processors),
      std::move(searcher), std::make_unique<result::CountLinesResult>());
}
...
std::shared_ptr<match_byte_offsets> extern_search(const std::string& pattern,
  ...
  return std::make_shared<match_byte_offsets>(
      num_threads, std::move(reader), std::move(processors),
      std::move(searcher), std::make_unique<result::MatchByteOffsetsResult>());
}
...
std::shared_ptr<line_byte_offsets> extern_search(const std::string& pattern,
  ...
  return std::make_shared<line_byte_offsets>(
      num_threads, std::move(reader), std::move(processors),
      std::move(searcher), std::make_unique<result::LineByteOffsetsResult>());
}
...
std::shared_ptr<line_indices> extern_search(const std::string& pattern,
  ...
  return std::make_shared<line_indices>(
      num_threads, std::move(reader), std::move(processors),
      std::move(searcher), std::make_unique<result::LineIndicesResult>());
}
...
std::shared_ptr<lines> extern_search(const std::string& pattern,
  ...
  return std::make_shared<lines>(num_threads, std::move(reader),
                                 ...
                                 std::make_unique<result::LinesResult>());
}
...
std::shared_ptr<count_matches> extern_search(const std::string& pattern,
  ...
  std::unique_ptr<task::base::DataProvider<DataChunk>> reader;
  if (num_readers == 1) {
    reader = std::make_unique<task::reader::FileBlockMetaReaderSingle>(
        file_path, meta_file_path);
  } else {
    if (ignore_case) {
      reader = std::make_unique<task::reader::FileBlockMetaReader>(
          file_path, meta_file_path, num_readers);
    } else {
      reader = std::make_unique<task::reader::FileBlockMetaReaderMMAP>(
          file_path, meta_file_path, num_readers);
    }
  }
  // check for compression and add decompression task --------------------------
