...
class BaseSearcher {
 public:
  ...
  virtual void search(const std::string& pattern, DataT* data, ResultT* result) const = 0;
  virtual void search(re2::RE2* pattern, DataT* data, ResultT* result) const = 0;
};
...
class MatchCounter : public BaseSearcher<DataChunk, PartialResult> {
 public:
  ...
  void search(const std::string& pattern, DataChunk* data, PartialResult* result) const override;
  void search(re2::RE2* pattern, DataChunk* data, PartialResult* result) const override;
};


class MatchBytePositionSearcher : public BaseSearcher<DataChunk, PartialResult> {
 public:
