...
class InlineBenchmark {
 ...
 public:
  ...
  virtual void stop() = 0;
  [[nodiscard]] virtual std::map<std::thread::id, int64_t> getResults()
      const = 0;

 protected:
  std::mutex _pushToResultPairsMutex;
  std::map<std::thread::id, std::vector<std::pair<TimePoint, TimePoint>>>
      _resultPairs;
};
...
class InlineBenchmarkWall
    : public InlineBenchmark<std::chrono::time_point<std::chrono::steady_clock>>

{
 public:
  ...
  void start() override {
    ...
    auto& res_vector = _resultPairs[std::this_thread::get_id()];
    if (res_vector.empty()) {
      res_vector.emplace_back(
          std::chrono::steady_clock::now(),
          std::chrono::time_point<std::chrono::steady_clock,
                                  std::chrono::nanoseconds>{});
      return;
    }
    if (res_vector.back().second ==
        std::chrono::time_point<std::chrono::steady_clock,
                                std::chrono::nanoseconds>{}) {
      throw std::runtime_error(
          "Starting dynamic benchmark failed, since there already is a timer "
...
std::ostream& operator<<(std::ostream& os, const InlineBenchmark<T>& c_bm) {
  os << c_bm._resultPairs.size() << " "
     << (c_bm._resultPairs.size() == 1 ? "thread" : "threads") << ":\n";
  double total = 0;
...
class InlineBenchmarkRegistrator {
 public:
  InlineBenchmarkRegistrator(std::string name, BenchmarkType bm_t_id)
      : _name(std::move(name)), _bm_t_id(bm_t_id) {
