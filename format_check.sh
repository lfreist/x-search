...
find ./src/ ./test/ ./include/ ./benchmarks -not \( -path ./include/xsearch/utils/ad_utility -prune \) -regextype egrep -regex '.*\.(h|c)(pp|xx)?$' -print0 >sourcelist
...
if [ "${#FAILED_FILES}" -eq 0 ]; then
  ...
  printf "\x1b[31mclang-format discovered style issues in the following files:\x1b[m\n"
